-- ============================================================
-- WHISPRR MESSAGING SYSTEM & RLS POLICIES FIX (ROBUST VERSION)
-- ============================================================
-- Copy all code below and run it in the Supabase SQL Editor.
...
CREATE POLICY "insert_conversations" ON public.conversations FOR INSERT TO public
WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "update_conversations" ON public.conversations;
CREATE POLICY "update_conversations" ON public.conversations FOR UPDATE TO public
USING (
  auth.uid() IS NOT NULL AND
  EXISTS (
    SELECT 1 FROM public.conversation_participants 
...
CREATE POLICY "delete_conversations" ON public.conversations FOR DELETE TO public
USING (
  auth.uid() IS NOT NULL AND
  EXISTS (
    SELECT 1 FROM public.conversation_participants 
...
CREATE POLICY "select_conversation_participants" ON public.conversation_participants FOR SELECT TO public
USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "insert_conversation_participants" ON public.conversation_participants;
CREATE POLICY "insert_conversation_participants" ON public.conversation_participants FOR INSERT TO public
WITH CHECK (
  auth.uid() IS NOT NULL AND (
    auth.uid() = user_id 
    OR EXISTS (
...
CREATE POLICY "delete_conversation_participants" ON public.conversation_participants FOR DELETE TO public
USING (
  auth.uid() IS NOT NULL AND (
    auth.uid() = user_id 
    OR EXISTS (
...
CREATE POLICY "select_messages" ON public.messages FOR SELECT TO public
USING (
  auth.uid() IS NOT NULL AND
  EXISTS (
    SELECT 1 FROM public.conversation_participants 
...
CREATE POLICY "insert_own_message" ON public.messages FOR INSERT TO public
WITH CHECK (
  auth.uid() IS NOT NULL AND
  auth.uid() = sender_id 
  AND EXISTS (
...
CREATE POLICY "update_messages_read" ON public.messages FOR UPDATE TO public
USING (
  auth.uid() IS NOT NULL AND
  EXISTS (
    SELECT 1 FROM public.conversation_participants 
