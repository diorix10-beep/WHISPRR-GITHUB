-- Migration: 033_v5_nexa_definitions.sql
-- Description: Adds rp_definition, system_definition, and system_character_definition columns to ai_characters and updates the create_ai_character RPC helper.

-- 1. Add definitions columns to public.ai_characters
ALTER TABLE public.ai_characters 
  ADD COLUMN IF NOT EXISTS rp_definition text DEFAULT '',
  ADD COLUMN IF NOT EXISTS system_definition text DEFAULT '',
  ADD COLUMN IF NOT EXISTS system_character_definition text DEFAULT '';

-- 2. Drop the old create_ai_character function
DROP FUNCTION IF EXISTS public.create_ai_character(
  text, text, text, text, text, text, text, text, text, text, text, text[], text, text, text, text, text, text, text
);

-- 3. Create updated create_ai_character function supporting definitions
