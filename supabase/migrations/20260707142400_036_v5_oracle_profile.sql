...
BEGIN
    ...
    -- We use a known static UUID for Oracle so it's consistent across environments
    ...
    -- 2. Insert or update Oracle's profile
    ...
    -- 3. We also need to add a "verified" or "system" badge if not existing, but 
    -- since badges are handled via `user_badges` and `badges` tables, let's just 
    -- rely on checking username = 'oracle' in the UI for now, or insert a badge.
END $$;
