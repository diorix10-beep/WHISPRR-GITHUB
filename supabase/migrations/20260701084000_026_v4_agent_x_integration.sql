-- Migration: 026_v4_agent_x_integration.sql
-- Description: Creates tables for AI agent activity logs, objectives, insights, recommendations, campaigns, drafts, and seeds configurations.

-- 1. Create agent_activity_log table
CREATE TABLE IF NOT EXISTS public.agent_activity_log (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  agent_id text NOT NULL, -- 'oracle', 'iris', 'atlas', 'athena', 'aegis', 'whisprr'
  platform text NOT NULL, -- 'x', 'discord', 'system'
  type text NOT NULL, -- 'tweet', 'reply', 'announcement', 'system'
  content text NOT NULL,
  external_id text, -- Tweet ID or Discord Message ID
  ref_id uuid, -- Reference to roadmap item, changelog, or journal entry
  created_at timestamptz NOT NULL DEFAULT now()
);

-- 2. Create agent_objectives table
...
-- 4. Create agent_recommendations table
...
-- 5. Create agent_campaigns table
...
-- 8. Create generic RLS policies (allow all authenticated/public actions)
DROP POLICY IF EXISTS "select_activity_log" ON public.agent_activity_log;
CREATE POLICY "select_activity_log" ON public.agent_activity_log FOR SELECT TO public USING (true);
DROP POLICY IF EXISTS "insert_activity_log" ON public.agent_activity_log;
CREATE POLICY "insert_activity_log" ON public.agent_activity_log FOR INSERT TO public WITH CHECK (true);
...
-- 9. Seed/Update default settings
INSERT INTO public.system_settings (key, value)
VALUES (
  'x_integration',
  '{"enabled": false, "active_agents": ["oracle"], "check_interval_mins": 30, "search_keywords": ["WHISPRR", "voice social", "AI companions", "anonymous messaging", "digital relationship"], "auto_post_roadmap": false, "auto_post_changelog": false, "auto_post_journal": false}'::jsonb
)
ON CONFLICT (key) 
DO UPDATE SET value = '{"enabled": false, "active_agents": ["oracle"], "check_interval_mins": 30, "search_keywords": ["WHISPRR", "voice social", "AI companions", "anonymous messaging", "digital relationship"], "auto_post_roadmap": false, "auto_post_changelog": false, "auto_post_journal": false}'::jsonb;
