...
import {
  X, Users, Globe, BookOpen, User, ArrowUpRight, Zap
} from 'lucide-react';
...
const whisprrUrl = isLocalhost
  ? 'http://localhost:5174'
  : 'https://whisprr.xyz';

const quickActions = [
  { label: 'Characters',  icon: Users,    path: '/characters', action: null, color: 'text-rose-500' },
  { label: 'Worlds',      icon: Globe,    path: '/worlds',     action: null, color: 'text-cyan-500' },
  { label: 'Lorebooks',   icon: BookOpen, path: '/lorebooks',  action: null, color: 'text-amber-500' },
  { label: 'Personas',    icon: User,     path: '/personas',   action: null, color: 'text-purple-500' },
] as const;
...
export function AppLauncherModal({ isOpen, onClose }: AppLauncherModalProps) {
  ...
  const handleSwitchToApp = (app: 'whisprr' | 'chimera') => {
    ...
    setTimeout(() => {
      if (app === 'whisprr') {
        window.location.href = whisprrUrl;
      } else {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed z-[9999] inset-x-4 bottom-24 sm:bottom-auto sm:inset-x-auto sm:left-[calc(var(--sidebar-w,224px)+1rem)] sm:top-1/2 sm:-translate-y-1/2 sm:w-[420px]"
          >
            <div className="bg-white/98 dark:bg-warm-925/98 backdrop-blur-2xl rounded-3xl border border-warm-200/60 dark:border-warm-800/70 shadow-2xl shadow-warm-950/20 dark:shadow-warm-950/60 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-warm-100/80 dark:border-warm-800/60">
                <div className="flex items-center gap-2">
                  <div className="w-5 h-5 rounded-full bg-gradient-to-br from-rose-500 to-amber-500 flex-shrink-0" />
